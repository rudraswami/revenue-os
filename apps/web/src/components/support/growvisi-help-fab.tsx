...
/**
 * Contextual setup help FAB — merchant onboarding only (not customer WhatsApp chat).
 * Phase B: FAQ + LLM assistant grounded in Growvisi setup docs.
 */
export function GrowvisiHelpFab({ context: contextOverride }: { context?: HelpFabContext }) {
  ...
  const llmAvailable = capabilities?.setupHelpLlm ?? false;
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="flex border-b border-border/60 p-1">
              <button
                type="button"
              ...
              {llmAvailable && (
                <button
                  ...
                  onClick={() => setTab("ask")}
                  className={cn(
                    "flex flex-1 items-center justify-center gap-1 rounded-lg px-3 py-2 text-xs font-semibold transition-colors",
                    tab === "ask"
                      ? "bg-accent text-white"
                      : "text-muted-foreground hover:bg-[#ecfdf5]/80",
                  )}
