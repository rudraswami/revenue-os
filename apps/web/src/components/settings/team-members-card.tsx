...
import { Crown, Mail, Trash2, UserPlus, UserRound, X } from "lucide-react";
...
export function TeamMembersCard() {
  ...
  const inviteableRoles = useMemo(
    () => INVITE_ROLES.filter((r) => myRole && canInviteRole(myRole, r)),
    [myRole],
  );
  const [inviteRole, setInviteRole] = useState<MembershipRole>(
    inviteableRoles.includes("AGENT") ? "AGENT" : inviteableRoles[0] ?? "AGENT",
  );
  ...
  const inviteMutation = useMutation({
    mutationFn: (payload: { email: string; role: MembershipRole }) =>
      apiFetch<{ sent: boolean; email: string; emailSent?: boolean }>("/organizations/invites", {
        method: "POST",
        token: token ?? undefined,
        body: JSON.stringify(payload),
      }),
    onSuccess: (res) => {
      setSent({ email: res.email, emailSent: res.emailSent !== false });
      setEmail("");
  ...
  const ti = (key: string) => t(`settings.tabs.teamInvite.${key}`);

  const inviteForm = canInvite ? (
    <div className="space-y-3">
      <p className="text-sm font-semibold text-foreground">{ti("title")}</p>
      <form
      ...
      >
        ...
        <Button
          ...
          disabled={inviteMutation.isPending || limits?.canInvite === false || !email.trim()}
        >
          <UserPlus className="h-4 w-4" />
          {inviteMutation.isPending ? ti("sending") : ti("send")}
        </Button>
      </form>
    </div>
  ) : null;

  const seatsUsed = limits
    ? limits.memberCount + limits.pendingInvites
    : 0;
    ...
    <div className="space-y-4">
        ...
        <div className="rounded-xl border border-border/70 bg-background/50 p-4">
          {limits && (
            <p className="mb-3 text-xs text-muted-foreground">
              {ti("seatsUsed")
                .replace("{used}", String(seatsUsed))
                .replace("{limit}", String(limits.limit))}
            </p>
          )}
          {inviteForm}
          ...
          {sent && (
            ...
            >
              {sent.emailSent
                ? ti("sent").replace("{email}", sent.email)
                : ti("sentNoEmail").replace("{email}", sent.email)}
            </p>
          )}
      ...
      <div className="overflow-hidden rounded-xl border border-border/80">
        <div className="hidden grid-cols-[minmax(0,1.4fr)_minmax(0,1.2fr)_100px_100px_72px] gap-3 border-b border-border/70 bg-background px-4 py-2.5 text-xs font-medium text-muted-foreground md:grid">
          ...
          <span>{ti("email")}</span>
          <span>{ti("role")}</span>
          <span>{ti("lastActive")}</span>
          <span className="text-right">{ti("actions")}</span>
        </div>
      ...
      {canInvite && (
        <div>
          <p className="mb-2 text-xs font-medium text-muted-foreground">{ti("pending")}</p>
          {invitesLoading ? (
            ...
            <ul className="divide-y divide-border/60 overflow-hidden rounded-xl border border-border/80">
              {invites!.map((inv) => (
                ...
                >
                  ...
                  {(isAdmin || canInviteRole(myRole, inv.role)) && (
                    <button
                      type="button"
      ...
      {!canInvite && myRole && (
        <p className="text-sm text-muted-foreground">
          {ti("yourRole")}{" "}
          <span className="font-semibold text-foreground">{ROLE_LABELS[myRole]}</span>.{" "}
          {ti("contactAdmin")}
        </p>
      )}
    </div>
