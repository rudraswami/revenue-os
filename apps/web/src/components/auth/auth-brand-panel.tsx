"use client";

import { useEffect, useState } from "react";
import { motion } from "framer-motion";
import { HandHelping, IndianRupee, ShieldCheck } from "lucide-react";
import { useAuthI18n } from "@/components/auth/auth-i18n";
import { Logo } from "@/components/marketing/logo";
import { HeroWhatsappPhone } from "@/components/marketing/illustrations/hero-whatsapp-phone";

const CYCLE_MS = 9000;

export function AuthBrandPanel() {
  const { t } = useAuthI18n();
  const [tick, setTick] = useState(0);

  useEffect(() => {
    const id = setInterval(() => setTick((n) => n + 1), CYCLE_MS);
    return () => clearInterval(id);
  }, []);

  return (
    <div className="auth-brand-panel relative hidden flex-col justify-between overflow-hidden p-10 xl:p-12 lg:flex">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_20%_0%,rgb(0_108_73/0.35),transparent_55%)]" />
      <div className="pointer-events-none absolute -right-24 bottom-0 h-80 w-80 rounded-full bg-[#25D366]/10 blur-3xl" />

      <Logo variant="light" showTagline className="relative z-10" />

      <div className="relative z-10 flex flex-1 flex-col items-center justify-center py-8">
        <div className="relative w-full max-w-[320px]">
          <HeroWhatsappPhone tick={tick} />

          <motion.div
            initial={{ opacity: 0, x: -12 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.4, duration: 0.5 }}
            className="absolute -left-2 top-8 z-20 rounded-xl border border-[#6cf8bb]/30 bg-[#0b1c30]/95 px-3 py-2.5 shadow-[0_12px_32px_rgb(0_0_0/0.35)] backdrop-blur-sm xl:-left-8"
          >
            <div className="flex items-center gap-2">
              <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#6cf8bb]/20 text-[#6cf8bb]">
                <HandHelping className="h-3.5 w-3.5" />
              </span>
              <div>
                <p className="text-[10px] font-medium text-white/55">{t("auth.brandFloatHandoff")}</p>
                <p className="text-xs font-bold text-[#6cf8bb]">{t("auth.brandFloatHandoffValue")}</p>
              </div>
            </div>
          </motion.div>

          <motion.div
            initial={{ opacity: 0, x: 12 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.55, duration: 0.5 }}
            className="absolute -right-2 bottom-16 z-20 rounded-xl border border-white/10 bg-white/95 px-3 py-2.5 shadow-[0_12px_32px_rgb(0_0_0/0.25)] backdrop-blur-sm xl:-right-10"
          >
            <div className="flex items-center gap-2">
              <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-accent/15 text-accent">
                <IndianRupee className="h-3.5 w-3.5" />
              </span>
              <div>
                <p className="text-[10px] font-medium text-muted-foreground">{t("auth.brandFloatPipeline")}</p>
                <p className="text-xs font-bold text-accent">{t("auth.brandFloatPipelineValue")}</p>
              </div>
            </div>
          </motion.div>
        </div>
      </div>

      <div className="relative z-10 max-w-md">
        <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#6cf8bb]/35 bg-[#6cf8bb]/10 px-3 py-1 text-xs font-bold text-[#6cf8bb]">
          {t("auth.brandBadge")}
        </div>
        <h2 className="text-2xl font-bold leading-tight tracking-tight text-white xl:text-[1.75rem]">
          {t("auth.brandHeadline")}
        </h2>
        <p className="mt-3 text-[14px] leading-relaxed text-white/70">{t("auth.brandBody")}</p>
        <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-white/60">
          <span className="inline-flex items-center gap-1.5">
            <ShieldCheck className="h-3.5 w-3.5 text-[#6cf8bb]" />
            {t("auth.brandStatMeta")}
          </span>
          <span>{t("auth.brandStatInr")}</span>
          <span>{t("auth.brandStatTrial")}</span>
        </div>
      </div>
    </div>
  );
}
